...
#if defined(NDEBUG)
# error "Faggotcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd16b29d3b53e3c432207279f3e5d1f2206b0360f5fb0ef988f647ab9da38a3f9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Faggotcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Faggotcoin Signed Message:\n";
...
{
    // Faggotcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Faggotcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Faggotcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Faggotcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Faggotcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Faggotcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1a09576ecf6cf8114cae44630091ad7cfdb8fa9bfd40a06c3f78b4b130139060");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "You're a faggot mate!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 69 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04967c0f4e208a675e6f5240adc97639120fe102dae4738cd7bff518a7de669ff197d5d786d470b7d63b13acd398bc8a3082cfa3e602bf059c9ddce1f580bc1fb3") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518325891;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085134882;

        if (fTestNet)
        {
            block.nTime    = 1518325879;
            block.nNonce   = 389704452;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xda8af9002f8beb99b0db77f13678ae691017cedfeabf5fabeccd27aafcd26594"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc3, 0xbb, 0xd1 }; // Faggotcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FaggotcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FaggotcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FaggotcoinMiner(CWallet *pwallet)
{
    printf("FaggotcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("faggotcoin-miner");
    ...
    try { loop {
        ...
        printf("Running FaggotcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FaggotcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FaggotcoinMiner, pwallet));
}
